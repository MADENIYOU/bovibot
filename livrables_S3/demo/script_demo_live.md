...
\*\*URL\*\* : http://<IP\_VPS>/ ou http://localhost/ (plan de secours)
...
\- \[ ] VPS accessible : `curl http://<IP\_VPS>/api/health` → `{"status":"ok"}`

\- \[ ] Données fraîches chargées (seed\_data.sql réinjecté si nécessaire)

\- \[ ] Navigateur ouvert en plein écran sur `http://<IP\_VPS>/`
...
> Il utilise nos fonctions fn\_gmq() et fn\_age\_en\_mois() pour répondre
...
> demande confirmation obligatoire, puis exécute sp\_enregistrer\_pesee().
...
> "Même principe pour les ventes. sp\_declarer\_vente() change le statut
